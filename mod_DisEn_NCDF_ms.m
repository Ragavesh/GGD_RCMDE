function [Out_DisEn, npdf]=mod_DisEn_NCDF_ms(x,m,nc,mu,sigma,tau,beta)
%
...
%pdf   
fy=(beta/(2*rho*gamma(1/beta)))*exp(-(abs(x - mu)/rho).^beta);
%cdf
c = 0.5*sign(x-mu);
%Fy=(0.5 + c) - c.*gammainc((1/beta),(abs(x-mu)/rho).^beta)/(2*gamma(1/beta));
%from python distribution line 6822
Fy=(c+0.5)-c.*gammainc((1.0/beta),(abs(x-mu)/rho)).^beta;
